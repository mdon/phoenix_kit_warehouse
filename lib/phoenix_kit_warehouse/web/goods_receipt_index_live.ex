defmodule PhoenixKitWarehouse.Web.GoodsReceiptIndexLive do
  @moduledoc """
  ...
  def handle_event("search", %{"search" => search}, socket) do
    {:noreply, socket |> assign(:search, search) |> assign_receipts()}
  end

  @impl true
  def handle_event("set_sort", %{"sort_by" => by}, socket) do
    {:noreply, socket |> assign(:sort_by, parse_sort_by(by)) |> assign_receipts()}
  end
  ...
  def handle_event("toggle_sort", %{"by" => by}, socket) do
    ...
    {:noreply,
     socket |> assign(:sort_by, sort_by) |> assign(:sort_dir, sort_dir) |> assign_receipts()}
  end

  @impl true
  def handle_event("flip_sort_dir", _params, socket) do
    {:noreply,
     socket |> assign(:sort_dir, flip_dir(socket.assigns.sort_dir)) |> assign_receipts()}
  end
  ...
  defp assign_receipts(socket) do
    receipts =
      GoodsReceipts.list_goods_receipts()
      |> enrich_receipts()
  ...
  defp enrich_receipts(receipts) do
    ...
    supplier_order_refs = DocRefs.supplier_order_refs(so_uuids)

    Enum.map(receipts, fn receipt ->
      supplier = Map.get(suppliers_by_uuid, receipt.supplier_uuid)
      so_ref = Map.get(supplier_order_refs, receipt.supplier_order_uuid)
      ...
      %{
        ...
        supplier_uuid: receipt.supplier_uuid,
        supplier_name: supplier && supplier.name,
        supplier_order_uuid: receipt.supplier_order_uuid,
        supplier_order_ref: so_ref,
        location_uuid: receipt.location_uuid,
        inserted_at: receipt.inserted_at,
        posted_at: receipt.posted_at,
        note: receipt.note,
        lines_count: length(receipt.lines || [])
      }
    end)
  end
  ...
  defp parse_sort_by(value) when is_binary(value) do
    case Map.get(GoodsReceiptColumnConfig.column_metadata_map(), value) do
      %{sortable?: true} -> value
      _ -> "number"
    end
  end
  ...
  defp default_dir(column_id) do
    case Map.get(GoodsReceiptColumnConfig.column_metadata_map(), column_id) do
      %{default_dir: dir} -> dir
      _ -> :asc
    end
  end
  ...
  defp apply_sort(entries, by, dir) do
    case Map.get(GoodsReceiptColumnConfig.column_metadata_map(), by) do
  ...
  def render(assigns) do
    ...
    >
      <div class="flex flex-col mx-auto max-w-none sm:px-4 py-2 sm:py-6 gap-2">
        <WarehouseHeader.warehouse_header active={:goods_receipts} />

        <.table_default
          id="goods-receipts-table"
          variant="zebra"
          size="sm"
          toggleable
          items={@receipts}
        ...
        >
          <:toolbar_title>
            <div class="flex flex-wrap items-center gap-2">
              <form id="gr-search" phx-change="search" class="contents">
              ...
              <%= for id <- @active_filters,
                        meta = Map.get(GoodsReceiptColumnConfig.column_metadata_map(), id),
                        meta do %>
          ...
          <.table_default_body>
            <%= if @receipts == [] do %>
              <.table_default_row hover={false}>
                ...
                >
                  <.icon name="hero-inbox-arrow-down" class="h-10 w-10 mx-auto mb-2 opacity-50" />
                  <div class="text-sm font-medium">{dgettext("default", "No goods receipts yet")}</div>
                </.table_default_cell>
              </.table_default_row>
            <% end %>
            <%= for entry <- @receipts do %>
              <.table_default_row class="relative cursor-pointer">
                ...
                <.table_default_cell class="relative z-10 w-12">
                  <.table_row_menu id={"gr-menu-#{entry.uuid}"}>
                    <.table_row_menu_link
                      navigate={
        ...
        <ColumnModal.column_modal
          ...
          column_config={GoodsReceiptColumnConfig}
  ...
  defp render_cell("supplier_order", entry) do
    case entry.supplier_order_ref do
      nil ->
        "—"
  ...
  defp render_card_value("supplier_order", entry) do
    case entry.supplier_order_ref do
      nil -> "—"
      ref -> ref.label
    end
